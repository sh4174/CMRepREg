...
::ReadModel(Registry &R)
{
  ...
  if(subtype == "PDE")
  {
	...
	PDESubdivisionMedialModel *smm;

    // Create the medial model
    try{
    	smm = new PDESubdivisionMedialModel;
    }
    catch(std::bad_alloc &ba){
        cerr << "Bad Alloc caught: " << ba.what() << endl;
    }
    catch (const std::overflow_error& e) {
        cerr << "Overflow caught: " << e.what() << endl;
    } catch (const std::runtime_error& e) {
        cerr << "Runtime Error Caught: " << e.what() << endl;
    } catch (const std::exception& e) {
        cerr << "Exception: " << e.what() << endl;
    } catch (...) {
        cerr << "Something wrong akldfjklasd" << endl;
    }
