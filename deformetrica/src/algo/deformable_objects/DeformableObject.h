...
public:
	...
	typedef enum
	{
		...
		CMRep			/*!< Continuous Medial Representation (see CMRep). */	
...
// inline void SetTimeIndex(unsigned int t) { m_TimeIndex = t; }
	...
	inline bool IsOfLandmarkKind() const { return ( (m_Type == Landmark) || (m_Type == PointCloud) ||
			...
			(m_Type == NonOrientedPolyLine) || (m_Type == NonOrientedSurfaceMesh) || (m_Type == CMRep) ); }
	...
	inline bool IsMesh() const {return ( (m_Type == OrientedPolyLine) || (m_Type == NonOrientedPolyLine) ||
			(m_Type == OrientedSurfaceMesh) || (m_Type == NonOrientedSurfaceMesh) || (m_Type == CMRep) ); }
	...
	/// Sets the type of the deformable object to CMRep.
	inline void SetCMRepType() { m_Type = CMRep;}
